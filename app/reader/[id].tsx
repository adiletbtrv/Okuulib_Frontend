...
export default function ReaderScreen() {
  ...
  const handleScroll = (e: { nativeEvent: { contentOffset: { y: number } } }) => {
    ...
    const shouldShow = diff < -4 || y < 40;
    if (shouldShow !== headerVisible) {
      setHeaderVisible(shouldShow);
      Animated.spring(headerAnim, { toValue: shouldShow ? 1 : 0, useNativeDriver: true, damping: 20, stiffness: 180 }).start();
    }
  };
  ...
  return (
    <View style={[s.flex, { backgroundColor: C.bg }]}>
      {/* Auto-hide top bar */}
      <Animated.View style={[s.topBar, { paddingTop: insets.top, transform: [{ translateY: headerAnim.interpolate({ inputRange: [0, 1], outputRange: [-(insets.top + 60), 0] }) }] }]}>
        ...
        <View style={s.topBarInner}>
          ...
          <View style={{ flex: 1, alignItems: "center" }}>
            <Text style={s.topTitle} numberOfLines={1}>{book.title}</Text>
            <Text style={s.topSub} numberOfLines={1}>{chapter?.chapterTitle ?? `Бөлүм ${chapterIdx + 1}`}</Text>
          </View>
          {/* Chapter count badge */}
          <View style={s.chapterBadge}>
            <Text style={s.chapterBadgeText}>{chapterIdx + 1} / {sortedChapters.length}</Text>
          </View>
        </View>
      </Animated.View>
      ...
      <ScrollView
        ref={scrollRef}
        style={s.flex}
        contentContainerStyle={{ paddingTop: insets.top + 68, paddingBottom: insets.bottom + 100, paddingHorizontal: 24 }}
      ...
      >
        ...
        {sortedChunks.map((chunk: ChunkResponse) => (
          <View key={chunk.chunkId} style={{ marginBottom: 16 }}>
            {chunk.chunkType === "image" ? (
              <Image
                source={{ uri: chunk.text || coverUrl || "" }}
                style={{ width: CONTENT_W, height: CONTENT_W * 0.6, borderRadius: 12 }}
                contentFit="cover"
              />
            ) : (
              <RenderHtml
...
const createStyles = (C: ThemeColors, isLight: boolean) => StyleSheet.create({
  flex: { flex: 1 },
  // Top bar
  topBar: { position: "absolute", top: 0, left: 0, right: 0, zIndex: 100, backgroundColor: isLight ? C.accent : "transparent" },
  topBarInner: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingBottom: 12, paddingTop: 4, gap: 12 },
  topBtn: { width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center", backgroundColor: isLight ? "rgba(255,255,255,0.2)" : "rgba(100,100,100,0.2)" },
  topTitle: { color: isLight ? "#FFFFFF" : C.offWhite, fontSize: 16, fontWeight: "700", letterSpacing: -0.2 },
  topSub: { color: isLight ? "rgba(255,255,255,0.8)" : C.muted, fontSize: 12, marginTop: 2 },
  chapterBadge: { backgroundColor: isLight ? "rgba(255,255,255,0.25)" : C.surface, borderRadius: 16, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: isLight ? "rgba(255,255,255,0.1)" : C.border },
  chapterBadgeText: { color: isLight ? "#FFFFFF" : C.mutedLight, fontSize: 12, fontWeight: "700", fontVariant: ["tabular-nums"] },
  // Chapter heading
  chapterHeading: { marginBottom: 28, paddingTop: 8 },
  chapterEyebrow: { color: C.accent, fontSize: 11, fontWeight: "600", letterSpacing: 1.4, textTransform: "uppercase", marginBottom: 8 },
  chapterName: { color: C.offWhite, fontSize: 26, fontWeight: "700", letterSpacing: -0.4, lineHeight: 32 },
  chapterRule: { height: 1, backgroundColor: C.border, marginTop: 20 },
  // Nav
  navRow: { flexDirection: "row", gap: 12, marginTop: 40 },
  navBtn: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, height: 50, borderRadius: 25, borderWidth: 1, borderColor: C.border, backgroundColor: C.surface },
  navBtnPrimary: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, height: 50, borderRadius: 25, backgroundColor: C.accent },
  navBtnDisabled: { opacity: 0.3 },
  navBtnText: { color: C.offWhite, fontWeight: "500", fontSize: 14 },
  navBtnPrimaryText: { color: C.white, fontWeight: "600", fontSize: 14 },
  // Back pill
  backPill: { backgroundColor: C.accent, paddingHorizontal: 24, paddingVertical: 12, borderRadius: 24 },
});